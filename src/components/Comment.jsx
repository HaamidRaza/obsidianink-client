...
export const Comment = ({ comment, blogId}) => {
  const { user } = useUser();
  ...
  return (
    <div
      className={`border-t border-soft pt-6 first:border-t-0 first:pt-0 ${comment.isPending ? "opacity-60" : ""}`}
    >
      {/* Comment Header */}
      <div className="flex items-start sm:items-center gap-3 mb-3">
        <CustomImage
          src={comment.user?.img || "default-image.jpg"}
          className="w-10 h-10 md:w-10 md:h-10 rounded-full object-cover border-2 border-soft shrink-0"
          width={45}
          height={45}
        />
        <div className="flex items-center gap-2 md:flex md:flex-row md:gap-4 ">
          <div className="flex flex-col md:flex md:flex-col flex-1 min-w-0">
            <span className="font-semibold text-ink text-sm md:text-base truncate">
              {comment.user?.username || "Anonymous"}
            </span>
            <span className="meta text-[8px] md:text-[10px]">
              {comment.isPending ? "Just now" : format(comment.createdAt)}
            </span>
          </div>
          {user &&
            (comment.user.username === user.fullName || role === "admin") && (
              <button
                onClick={() => mutation.mutate()}
                disabled={mutation.isPending}
                className="md:border md:border-soft md:rounded-2xl text-red-400 text-left hover:text-red-300 flex items-center group transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed px-2 py-0.5"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 100 100"
                  className="w-6 h-6 shrink-0 group-hover:accent transition-colors"
                  fill="currentColor"
                >
      ...
      {/* Comment Body */}
      <div className="ml-2 md:ml-13 pl-3 border-l md:border-l-2 border-soft">
        <p className="text-muted leading-relaxed text-sm md:text-lg">
          {comment.desc}
          {comment.isPending && (
            <span className="ml-2 text-xs italic">(Posting...)</span>
          )}
        </p>
      </div>
    </div>
